...
public class Sorts {
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> bubbleSort(T[] arr) {
        ...
        while (arrayinorder == false) {
            ...
            for (int iter = 0; iter < (arr.length - 1); iter++) {
                l.add(new CompareEvent(iter, iter + 1));
                if (arr[iter].compareTo(arr[iter + 1]) > 0) {
                    l.add(new SwapEvent(iter, iter + 1));
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> insertionSort(
        ...
        for (int iter = 0; iter < arr.length; iter++) {
            ...
            for (iter2 = 0; iter2 < iter; iter2++) {
                l.add(new CompareEvent(iter, iter2));
                if (save.compareTo(arr[iter2]) < 0) {
                    break;
                }
            }
            for (int iter3 = iter; iter3 > iter2; iter3--) {
                l.add(new CopyEvent(iter3, arr[iter3 - 1]));
                arr[iter3] = arr[iter3 - 1];
            }
            l.add(new CopyEvent(iter2, save));
            arr[iter2] = save;
        }
        return l;
    }
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> mergeSort(T[] arr) {
        List<SortEvent<Integer>> l = new ArrayList<SortEvent<Integer>>();
        mergeSortHelper(arr, arr, 0, arr.length, l);
        return null;
    }
    ...
    public static <T extends Comparable<? super T>> void 
        mergeSortHelper(T[] arr, T[] arr2, int lo, int hi, List<SortEvent<Integer>> l) {        
        ...
        mergeSortHelper(arr2, arr2, lo, midPoint, l);
        mergeSortHelper(arr2, arr2, midPoint + 1, hi, l);
        
        for (int i = lo; i < hi; i++) {
            l.add(new CompareEvent(pointerFir, midPoint));
            if (pointerFir == midPoint) {
                l.add(new CopyEvent<T>(pointerSec, arr[pointerSec]));
                arr2[i] = arr[pointerSec];
                pointerSec++;
            } else if (pointerSec == hi) {
                l.add(new CompareEvent(pointerSec, hi));
                l.add(new CopyEvent<T>(pointerSec, arr[pointerSec]));
                arr2[i] = arr[pointerFir];
                pointerFir++;
            } else if (arr[pointerFir].compareTo(arr[pointerSec]) < 0) {
                l.add(new CompareEvent(pointerFir, pointerSec));
                l.add(new CopyEvent<T>(pointerSec, arr[pointerSec]));
                arr2[i] = arr[pointerFir];
                pointerFir++;
            } else if (arr[pointerSec].compareTo(arr[pointerFir]) < 0) {
                l.add(new CompareEvent(pointerSec, pointerFir));
                l.add(new CopyEvent<T>(pointerSec, arr[pointerSec]));
                arr2[i] = arr[pointerSec];
                pointerSec++;
            } 
     ...
     * @param lo inclusive
     * @param l 
     * @return l SortEvents list
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> 
        quickSortHelper(T[] arr, int lo, int hi, List<SortEvent<Integer>> l) {
        int pivot = hi;
        int pointerLo = lo;
        int pointerHi = hi - 1;
        boolean cont = true;
